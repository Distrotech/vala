...
static void vala_parser_parse_local_constant_declarations (ValaParser* self, ValaBlock* block, GError** error) {
	...
	}
	{
		...
		while (TRUE) {
			...
			ValaDeclarationStatement* _tmp20_;
			ValaBlock* _tmp21_;
...
static void vala_parser_parse_property_declaration (ValaParser* self, ValaSymbol* parent, ValaList* attrs, GError** error) {
	...
	if (_tmp80_) {
		...
		ValaSourceReference* _tmp90_;
		{
			...
			while (TRUE) {
				...
				_tmp81_ = FALSE;
				_tmp84_ = vala_parser_parse_type (self, TRUE, FALSE, &_inner_error_);
				_tmp85_ = _tmp84_;
				if (_inner_error_ != NULL) {
					if (_inner_error_->domain == VALA_PARSE_ERROR) {
						g_propagate_error (error, _inner_error_);
	...
	while (TRUE) {
		...
		if (_tmp92_) {
			...
			_tmp100_ = vala_parser_parse_expression (self, &_inner_error_);
			_tmp101_ = _tmp100_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == VALA_PARSE_ERROR) {
					g_propagate_error (error, _inner_error_);
			...
			if (_tmp113_) {
				...
				if (!_tmp123_) {
					...
					_tmp124_ = vala_parser_parse_block (self, &_inner_error_);
					_tmp125_ = _tmp124_;
					if (_inner_error_ != NULL) {
						if (_inner_error_->domain == VALA_PARSE_ERROR) {
							g_propagate_error (error, _inner_error_);
				...
				if (!_tmp159_) {
					...
					_tmp160_ = vala_parser_parse_block (self, &_inner_error_);
					_tmp161_ = _tmp160_;
					if (_inner_error_ != NULL) {
						if (_inner_error_->domain == VALA_PARSE_ERROR) {
							g_propagate_error (error, _inner_error_);
	...
	if (!_tmp185_) {
		...
		ValaSourceFileType _tmp187_;
		ValaSourceFileType _tmp188_;
		_tmp186_ = prop;
	...
	if (_tmp189_) {
		...
		if (_tmp224_) {
			...
			ValaMemberBinding _tmp251_;
			ValaMemberBinding _tmp252_;
			_tmp225_ = prop;
