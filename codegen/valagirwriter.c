...
static void vala_gir_writer_write_param_or_return (ValaGIRWriter* self, ValaDataType* type, gboolean is_parameter, gint* index, gboolean has_array_length, const gchar* name, const gchar* comment, ValaParameterDirection direction, gboolean constructor, gboolean caller_allocates) {
	...
	if (_tmp30_) {
		...
		any_owned = FALSE;
		{
			...
			ValaList* _generic_arg_list;
			ValaList* _tmp33_;
			gint _tmp34_;
			gint _tmp35_;
			gint _generic_arg_size;
			gint _generic_arg_index;
			_tmp31_ = type;
	...
	vala_gir_writer_write_type (self, _tmp99_, _tmp100_);
	_tmp101_ = self->priv->indent;
	self->priv->indent = _tmp101_ - 1;
	vala_gir_writer_write_indent (self);
	_tmp102_ = self->priv->buffer;
	_tmp103_ = tag;
	g_string_append_printf (_tmp102_, "</%s>\n", _tmp103_);
	_tmp104_ = *index;
	*index = _tmp104_ + 1;
	...
	if (G_TYPE_CHECK_INSTANCE_TYPE (_tmp4_, VALA_TYPE_NAMESPACE)) {
		...
		if (_tmp17_ != NULL) {
			...
			if (_tmp24_ != NULL) {
				...
				_tmp44_ = type_symbol;
				_tmp45_ = vala_code_node_get_attribute_string ((ValaCodeNode*) _tmp44_, "GIR", "name", NULL);
				_tmp46_ = _tmp45_;
				_tmp47_ = _tmp46_;
				if (_tmp47_ == NULL) {
